...
object ModuloExterno extends IModuloExterno {
  ...
  override def combinan(a: Transporte, b: Transporte): (Boolean, Option[Direccion]) = {
    ...
    (a, b) match {
      case (Colectivo("25",_), Tren("A", _)) => (true, Some(CDirs.A_000))
      case (Tren("A", _), Colectivo("25", _)) => (true, Some(CDirs.A_000))

      case (Colectivo("25", _), Colectivo("107", _)) => (true, Some(CDirs.A_700))
      case (Colectivo("107", _), Colectivo("25", _)) => (true, Some(CDirs.A_700))

      case (Tren("A", _), Subte("B", _)) => (true, Some(CDirs.B_000))
      case (Subte("B", _), Tren("A", _)) => (true, Some(CDirs.B_000))

      case (Colectivo("107", _), Subte("B", _)) => (true, Some(CDirs.B_400))
      case (Subte("B", _), Colectivo("107", _)) => (true, Some(CDirs.B_400))
      case (Colectivo("135", _), Subte("B", _)) => (true, Some(CDirs.B_400))
      case (Subte("B", _), Colectivo("135", _)) => (true, Some(CDirs.B_400))
      case (Colectivo("107", _), Colectivo("135", _)) => (true, Some(CDirs.B_400))
      case (Colectivo("135", _), Colectivo("107", _)) => (true, Some(CDirs.B_400))
      case (Colectivo("53", _), Colectivo("135", _)) => (true, Some(CDirs.B_400))
      case (Colectivo("135", _), Colectivo("53", _)) => (true, Some(CDirs.B_400))
  ...
  def getEstaciones(t: Transporte): List[Direccion] = {
    t match {
      case Colectivo("25", _) => List(
        CDirs.A_000, CDirs.A_200, CDirs.A_700)
      case Colectivo("107", _) => List(
        CDirs.A_700, CDirs.B_400)
      case Colectivo("53", _) => List(
        CDirs.A_200, CDirs.B_400)
      case Colectivo("135", _) => List(
